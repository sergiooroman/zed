...
pub(crate) enum CommitContextMenuSource {
    // The graph builds its own menu inline, because this fork adds merge,
    // cherry-pick, revert, checkout and tag entries that this shared menu does
    // not model. Kept so the two stay in sync if that menu is ever adopted.
    #[allow(dead_code)]
    GitGraph,
    GitPanel,
}
