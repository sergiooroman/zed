...
impl GitPanel {
    ...
    pub fn select_entry_by_path(
        ...
        // Match on (repo_id, repo_path) first: in the aggregated view the same
        // relative path can exist in several repos, and a path-only lookup would
        // land on whichever comes first. Fall back to the section-aware lookups,
        // which also cover tree entries that carry no status of their own.
        let Some(ix) = self
            .entries
            .iter()
            .position(|entry| {
                entry.status_entry().is_some_and(|status| {
                    status.repo_id == repo_id && status.repo_path == repo_path
                })
            })
            .or_else(|| section.and_then(|section| self.entry_by_path_in_section(&repo_path, section)))
    ...
    fn open_diff(&mut self, _: &menu::Confirm, window: &mut Window, cx: &mut Context<Self>) {
        ...
        maybe!({
            ...
            let workspace = self.workspace.upgrade()?;
            // Resolve the repo from the entry rather than the active one: in the
            // aggregated view the selection can belong to any open repository.
    ...
    fn update_visible_entries(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if show_all_repositories {
            self.build_all_repos_entries(sort_by, group_by_file_status, path_style, window, cx);
            return;
        }
    ...
    fn build_all_repos_entries(
        ...
        for repo_entity in repos {
            ...
            let mut push_status = |this: &mut Self, entry: GitListEntry, section: Section| {
                if let Some(estimate) =
            ...
            // ordering within the repo, and record each entry under its section
            // so the path lookups stay section-aware.
            for (entries, section) in [
                (conflict_entries, Section::Conflict),
                (changed_entries, Section::Tracked),
                (new_entries, Section::New),
            ] {
                for entry in entries {
                    push_status(self, GitListEntry::Status(entry), section);
                }
            }
        }
