...
impl DiffMultibuffer {
    ...
    pub(crate) fn move_to_entry(
        ...
        // The git panel's aggregated "all repositories" view can select an entry
        // belonging to any open repo, so point the diff at that entry's
        // repository before resolving the path — otherwise the diff, and the
        // active-repo selector that follows it, stay on the previous repo.
        if self.repo(cx).map(|repo| repo.read(cx).id) != Some(entry.repo_id)
            && let Some(repo) = self
                .workspace
